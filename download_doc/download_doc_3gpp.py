...
download_urls = get_url(excel_dir, "3gpp_doc_list.xlsx", rows)
download_filenames = get_name_3gpp(excel_dir, "3gpp_doc_list.xlsx", rows)
...
for i, (download_url, download_filename) in enumerate(zip(download_urls, download_filenames), start=1):
    rep += 1  # 進行カウント
    if not file_exists_in_folder(download_path, str(rows[rep-1]) + "_" + download_filename + ".zip"):
